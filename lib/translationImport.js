...
var splitLanguages = function splitLanguages(data) {
  var languagesMap = {};
  var languagesList = data[0].slice(1).filter(String); // array with langs, first column is "code"
...
function translationImport(clientId, secret, spreadsheetId) {
  ...
  return googleSpreadsheet.authorize().then(function () {
    ...
    return googleSpreadsheet.readSpreadsheet(spreadsheetId, sheet + '!A1:Z');
  }).then(function (spreadsheetData) {
    var languagesMap = splitLanguages(spreadsheetData.data.values);
